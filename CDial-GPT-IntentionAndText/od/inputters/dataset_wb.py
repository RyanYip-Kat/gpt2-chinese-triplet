...
SPECIAL_TOKENS = ["[CLS]", "[SEP]", "[speaker1]","[intent]","[speaker2]"]
...
class WBDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.lm_labels:
            history = self.data[index][-2 * self.max_history:-2]
            intention = self.data[index][-2]
            resposne = self.data[index][-1]
        else:
            history = self.data[index][-2 * self.max_history:-2]
            intention = self.data[index][-2]
            resposne = []
        return self.process(history, intention,resposne)

    def process(self, history,intention,resposne, with_eos=True):
        bos, eos, speaker1,intent,speaker2 = self.tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)

        sequence = [[bos]] + history + [intention + ([eos] if with_eos else [])] + [resposne + ([eos] if with_eos else [])]
