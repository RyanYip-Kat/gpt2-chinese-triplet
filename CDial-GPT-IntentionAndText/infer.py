...
SPECIAL_TOKENS = ["[CLS]", "[SEP]", "[PAD]", "[speaker1]","[intent]", "[speaker2]"]
...
def build_input_from_segments(history, intention,reply, tokenizer, with_eos=True):
    """ Build a sequence of input from 3 segments: persona, history and last reply """
    bos, eos, pad, speaker1,intent, speaker2 = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)
    sequence = [[bos]] + history + [intention + ([eos] if with_eos else [])] + [reply + ([eos] if with_eos else [])]
    tmp = []
    for i, s in enumerate(sequence[1:]):
            if i%3==0:
                tmp.append([speaker2] + s)
            elif i%3==1:
                tmp.append([intent]  + s)
            else:
                tmp.append([speaker1] + s)
    sequence = [sequence[0]] + tmp
    ...
    for i,s,in enumerate(sequence[1:]):
             for _ in s:
                 if i%3==0:
                     token_type_ids.append(speaker2)
                 elif i%3==1:
                     token_type_ids.append(intent)
                 else:
                     token_type_ids.append(speaker1)
...
def sample_sequence(history,intention, tokenizer, model, args, current_output=None):
    special_tokens_ids = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)
    if current_output is None:
        current_output = []

    for i in range(args.max_length):
        instance, sequence = build_input_from_segments(history,intention, current_output, tokenizer, with_eos=False)
...
def main():
    ...
    for instance in tqdm(dataset, mininterval=1):
        history = tokenize(instance[:-2])
        intention = tokenize(instance[-2])
        with torch.no_grad():
            out_ids = sample_sequence(history,intention, tokenizer, model, args)
